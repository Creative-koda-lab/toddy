...
interface EditTodoModalProps {
  ...
  todo: Todo | null;
  onClose: () => void;
  onSave: (id: string, title: string, description: string, tags: TodoTag[]) => void;
}

/**
 * EditTodoModal Component
 *
 * Modal dialog for editing an existing todo item's title, description, and tags.
 */
export function EditTodoModal({ isOpen, todo, onClose, onSave }: EditTodoModalProps) {
  ...
  /**
   * Handles form submission and saves changes
   */
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (title.trim()) {
      onSave(todo.id, title, description, selectedTags);
      onClose();
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center md:p-4 z-50">
      <div className="bg-white md:rounded-2xl shadow-2xl max-w-2xl w-full h-full md:h-auto p-6 md:p-10 overflow-y-auto">
        <form onSubmit={handleSubmit}>
          ...
          {/* Title Input */}
          <div className="mb-8">
            <label htmlFor="edit-todo-title" className="block text-xl font-semibold text-gray-700 mb-3">
              Title
            </label>
            <input
              id="edit-todo-title"
          ...
          {/* Description Input */}
          <div className="mb-8">
            <label htmlFor="edit-todo-description" className="block text-xl font-semibold text-gray-700 mb-3">
              Description
            </label>
            <textarea
